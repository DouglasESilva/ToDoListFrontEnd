...
const PerfilUsuario = () => {
    ...
    const savePerfilUsuario = () => {
        ...
        console.log("Antes de salvar:", perfilUsuario);
        setSubmitted(true);
           ...
           perfilUsuarioService.alterar(perfilUsuario)
                .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'info',
                        summary: 'Sucesso',
                        detail: 'Perfil-Usuairo alterado com sucesso!'
                    })
                }).catch((error) => {
                    console.log(error)
                    toast.current?.show({
    ...
    const deletePerfilUsuario = () => {
        console.log("Deletando perfil-usuario:", perfilUsuario.id);
    ...
    const deleteSelectedPerfisUsuario = () => {
        ...
        Promise.all(selectedPerfisUsuario.map(async (_perfilUsuario) => {
            if(_perfilUsuario.id != undefined){
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="Mostrando {first} até {last} de {totalRecords} Perfis-Usuario"
                        globalFilter={globalFilter}
                        emptyMessage="Nenhum perfil usuario encontrado."
                        header={header}
                        responsiveLayout="scroll"
                    >
                        <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}></Column>
                        <Column field="id" header="Código" sortable body={idBodyTemplate} headerStyle={{ minWidth: '15rem' }}></Column>
                        <Column field="perfil.descricao" header="Perfil" sortable body={perfilBodyTemplate} headerStyle={{ minWidth: '15rem' }}></Column>
                        <Column field="usuario.nome" header="Usuário" sortable body={usuarioBodyTemplate} headerStyle={{ minWidth: '15rem' }}></Column>
                        
                        
                        <Column body={actionBodyTemplate} headerStyle={{ minWidth: '10rem' }}></Column>
                    </DataTable>

                    <Dialog visible={perfilUsuarioDialog} style={{ width: '450px' }} header="Detalhes de Perfis Usuairo" modal className="p-fluid" footer={perfilUsuarioDialogFooter} onHide={hideDialog}>
                        <div className="field">
                            <label htmlFor="descricao">Perfil</label>
                        ...
                        <div className="field">
                            <label htmlFor="descricao">Usuário</label>
                    ...
                    <Dialog visible={deletePerfilUsuarioDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletePerfilUsuarioDialogFooter} onHide={hideDeletePerfilUsuarioDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {perfilUsuario && (
                                <span>
                                    Tem certeza de que deseja excluir? <b>{perfilUsuario.id}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deletePerfisUsuarioDialog} style={{ width: '450px' }} header="Confirm" modal footer={deletePerfisUsuarioDialogFooter} onHide={hideDeletePerfisUsuarioDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {perfilUsuario && <span>Tem certeza de que deseja excluir os perfis-usuarios selecionados?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
